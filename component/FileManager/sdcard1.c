...
esp_err_t sdcard_readDataFromFile(long int *index_data ,const char *nameFile, const char *format, ...)
{
    ...
    char dataStr[256];
    char *returnPtr;
    //di chuyển con tro den vi tri returnPtr
    // ESP_LOGI(__func__,"dia chi returnPtr = %p",returnPtr);
    fseek(file, *index_data, SEEK_SET);
    returnPtr = fgets(dataStr, sizeof(dataStr), file);
    // returnPtr = returnPtr + sizeof(returnPtr);
    // ESP_LOGI(__func__,"dia chi returnPtr lan2  = %p",returnPtr);
    // returnPtr = fgets(dataStr, sizeof(dataStr), file);
    *index_data = ftell(file);
    fclose(file);
    // if (fseek(file, returnPtr, SEEK_SET) != 0) {
    //     printf("Lỗi khi di chuyển con trỏ tập tin.\n");
    //     fclose(file);
